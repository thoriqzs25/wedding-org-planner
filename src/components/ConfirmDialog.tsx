      ...
      <div className="relative bg-white rounded-2xl shadow-xl max-w-sm w-full mx-4 p-6 text-center space-y-4">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel}
            className="cursor-pointer flex-1 min-h-[44px] rounded-xl border border-gold/40 text-amber-900 font-medium hover:bg-cream transition-colors active:scale-[0.98]">
            {cancelLabel}
          </button>
          <button onClick={onConfirm}
            className={`cursor-pointer flex-1 min-h-[44px] rounded-xl text-white font-medium transition-colors active:scale-[0.98] ${
              variant === "danger"
