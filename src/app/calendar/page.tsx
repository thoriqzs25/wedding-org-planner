  ...
  return (
    <div id="calendar-page" className="max-w-5xl space-y-6">
      ...
      <div id="calendar-card" className="bg-white rounded-2xl border border-gold/30 shadow-sm overflow-hidden">
        ...
        <div id="calendar-day-names" className="grid grid-cols-7 border-b border-gold/20">
          {dayNames.map((d) => (<div key={d} id={`calendar-day-name-${d}`} className="p-3 text-center text-xs font-medium text-amber-800/50">{d}</div>))}
        </div>

        <div id="calendar-grid-wrapper" className="overflow-x-auto pb-2">
          <div id="calendar-grid" className="grid grid-cols-7 min-w-[490px] sm:min-w-0">
            {Array.from({ length: firstDay }).map((_, i) => (<div key={`empty-${i}`} id={`calendar-empty-${i}`} className="p-3" />))}
            {Array.from({ length: daysInMonth }).map((_, i) => {
              const day = i + 1;
              const dayEvents = getEventsForDay(day);
              const isToday = day === today.getDate() && currentMonth === today.getMonth() && currentYear === today.getFullYear();
              return (
                <div key={day} id={`calendar-day-${day}`} className={`p-2 min-h-[80px] sm:min-h-[90px] border-b border-r border-gold/10 hover:bg-cream/50 transition-colors ${isToday ? "bg-orange/5" : ""}`}>
                  <span id={`calendar-day-${day}-number`} className={`inline-flex w-7 h-7 items-center justify-center text-sm rounded-full ${isToday ? "bg-orange text-white font-semibold" : "text-amber-900"}`}>{day}</span>
                  <div id={`calendar-day-${day}-events`} className="mt-1 space-y-1">
                    {dayEvents.map((event) => (
                      <div key={event.id} id={`calendar-day-event-${event.id}`}
      ...
      <div id="calendar-event-list" className="bg-white rounded-2xl border border-gold/30 p-5 shadow-sm">
        ...
        {events.length === 0 ? (
          <div id="calendar-event-list-empty" className="text-center py-10 text-amber-800/40">
            ...
            <p id="calendar-event-list-empty-text" className="text-sm">Belum ada jadwal</p>
          </div>
        ) : (
          <div id="calendar-event-list-items" className="space-y-3">
            {events.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map((event) => (
              <div key={event.id} id={`calendar-event-item-${event.id}`}
                ...
                onClick={() => { setEditingEvent(event); setShowForm(true); }}>
                <div id={`calendar-event-item-${event.id}-date`} className="text-center min-w-[48px]">
                  <p id={`calendar-event-item-${event.id}-day`} className="text-lg font-bold text-orange leading-tight">{new Date(event.date).getDate()}</p>
                  <p id={`calendar-event-item-${event.id}-month`} className="text-[10px] text-amber-800/50">{monthNames[new Date(event.date).getMonth()].slice(0, 3)}</p>
                </div>
